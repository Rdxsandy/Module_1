"""
routers/events.py - Vehicle event ingestion + search.

POST /api/events  - AUTHENTICATED (any user can submit events)
                    Queues the event for background processing (Celery)
                    and returns immediately; the worker validates the
                    camera, persists the event, and raises an alert on a
                    watchlist match. See app/worker.py.
GET  /api/events  - AUTHENTICATED
"""
from typing import Optional
from datetime import datetime
from fastapi import APIRouter, Depends, Query
from sqlalchemy.orm import Session

from app.database import get_db
from app.schemas.event import EventCreate, EventOut, EventAckResponse
from app.worker import process_vehicle_event_task
from app.auth.dependencies import get_current_user
from app.models.user import User

router = APIRouter(prefix="/api/events", tags=["events"])


@router.post("", response_model=EventAckResponse, status_code=202)
def ingest_event(
    payload: EventCreate,
    _: User = Depends(get_current_user),
):
    """Queue a vehicle detection event for background processing. Requires login."""
    process_vehicle_event_task.delay(payload.model_dump_json())
    return EventAckResponse()


@router.get("", response_model=list[EventOut])
def search_events(
    vehicle_number: Optional[str] = Query(None),
    camera_id: Optional[int] = Query(None),
    start_time: Optional[datetime] = Query(None),
    end_time: Optional[datetime] = Query(None),
    limit: int = Query(100, le=500),
    db: Session = Depends(get_db),
    _: User = Depends(get_current_user),
):
    """Search events with optional filters. Requires login."""
    from app.models.vehicle_event import VehicleEvent
    from app.services.watchlist_service import normalize_vehicle_number

    q = db.query(VehicleEvent)
    if vehicle_number:
        q = q.filter(VehicleEvent.vehicle_number == normalize_vehicle_number(vehicle_number))
    if camera_id:
        q = q.filter(VehicleEvent.camera_id == camera_id)
    if start_time:
        q = q.filter(VehicleEvent.event_time >= start_time)
    if end_time:
        q = q.filter(VehicleEvent.event_time <= end_time)
    return q.order_by(VehicleEvent.event_time.desc()).limit(limit).all()
